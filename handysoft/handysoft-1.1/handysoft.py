...
class Logitheque(Gtk.Window):
    ...
    def __init__(self, tosearch):
        ...
        stack.add_titled(self.logibox, _("Software"), _("Software"))
        ...
        # lance mise à jour
        upgradebtn = Gtk.Button(_("Check for updates"))
        upgradebtn.set_tooltip_text(_("Check for updates"))
        upgradebtn.connect("button_release_event", lambda x,y: update_checker())
        upgradebtn.connect("activate", lambda x: update_checker())

        # conteneur de boutons
        btncontainer = Gtk.HBox()
        btncontainer.pack_start(stack_switcher, False, False, 0)
        btncontainer.pack_start(upgradebtn, False, False, 1)
    ...
    def open_cache(self):
        ...
        self.cachefiltered = [p for p in self.cache if trie_paquet(p.shortname, p.section) and not p.name.endswith(":i386")]
...
you must use the Synaptic Package Manager.
            ...
            elif c['section'] == 'hlfav': # sélection handylinux
                scroll = Gtk.ScrolledWindow()
                scroll.show()
                scroll.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)

                store = Gtk.ListStore(str, str, str, GdkPixbuf.Pixbuf) # lignes de la liste de paquets
                tree = Gtk.TreeView(store)
                tree.set_activate_on_single_click(True)
                tree.set_rules_hint(True)
                tree.connect("row_activated", self.show_pkg, store, 1) # au double clic, on montre des infos sur le paquet
                tree.show()
                ...
                renderer_pixbuf.set_padding(10,10)
                renderer_pixbuf.set_property("cell-background", "#333333")
                column_pixbuf = Gtk.TreeViewColumn(_("Screenshot"), renderer_pixbuf, pixbuf=3)
                tree.append_column(column_pixbuf)

                # les résultats sont affichés au fur et à mesure qu'ils sont trouvés
                task = self.fill_logitheque(store, hlfavs, screenshot=True, desc_perso=True)
                GObject.idle_add(next, task) 

                scroll.add(tree) # on met le tableau dans le scroll pour avoir la barre de défilement
            ...
            if c['section'] == 'hlfav' and os.path.isfile(c['icon']): # icone du handymenu
                pixbuf = GdkPixbuf.Pixbuf.new_from_file(c['icon']) # redimensionnement
                scaled_buf = pixbuf.scale_simple(iconsize,iconsize,GdkPixbuf.InterpType.BILINEAR)
                o = Gtk.Image.new_from_pixbuf(scaled_buf)
            else:
